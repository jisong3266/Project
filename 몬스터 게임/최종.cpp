...
int main() {
  srand(time(NULL));
  int num, count = 0, flag = 0, character_num, character_num2[2],
           weapon_number = 0, random_number, box, round = 1, game_type,
           game_level = 1, game_count;
  ...
  case 0: {
    ...
    while (1) {
      ...
      case 2: {
        ...
        if (weapon_choice == -2) {
          game_level++;
          character_bag[character_num]->hp += 10;
          g.power += 5;
          s.power += 5;
          continue;
        }
  ...
  case 1: {
    ...
    while (1) {
      ...
      box = game_level;
      if (current_player == 1 && player1_alive)
        cout << "\033[38;5;219mPlayer1\033[38;5;15m" << endl;
      else if (current_player == 2 && player2_alive)
      ...
      switch (num) {
      case 1: {
        character_num2[current_player - 1] =
            ChooseCharacter(character_bag, count);
        break;
      }
      case 0: {
        for (int i = 0; i < 2; i++) {
      ...
      case 2: {
        int weapon_choice =
            ChooseWeapon(character_bag, character_num2[current_player - 1],
                         game_count, round, w, g, s, m1);
        if (weapon_choice == -1) {
          continue;
        }
        if (weapon_choice == -2) {
          game_level++;
          character_bag[character_num2[0]]->hp += 10;
          g.power += 5;
          s.power += 5;
          character_bag[character_num2[1]]->hp += 10;
          continue;
        }
        game_count = 1;
        break;
      ...
      case 3: {
        game_count = 1;
        GainWeapon(character_bag, character_num2[current_player - 1], g, s);
        break;
      }
      case 4: {
        flag = 1;
      ...
      if (box == game_level && game_count == 1) {
        ...
        if (player1_alive && player2_alive) {
          character_bag[character_num2[current_player - 1]]->hp -=
              m1->attack_power;
          if (character_bag[character_num2[current_player - 1]]->hp <= 0) {
            if (current_player == 1) {
              cout << "Player1이 죽었습니다!" << endl;
              character_bag[character_num2[0]]->hp = 0;
              player1_alive = false;
              current_player = 2;
            } else {
              cout << "Player2가 죽었습니다!" << endl;
              player2_alive = false;
          ...
          character_bag[character_num2[current_player - 1]]->hp -=
              m1->attack_power;
          cout << "남은 HP : "
               << character_bag[character_num2[current_player - 1]]->hp << endl;
        }
      }
      if (player1_alive && player2_alive) {
        current_player = 3 - current_player; // 플레이어 차례 변경
      }
    }
  }
  }
